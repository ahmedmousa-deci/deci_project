...
def paved_road(name):
    f.text_animation(
        f"{name} started walking on the paved road,\n and after a short" +
        " while he met an old lady asking for help carrying her basket")
    ...
    return f.choice_input(["You decide to: Help the old lady.",
                          "You decide to: Apologize and move on."], [1, 2])
...
def narrow_path(name):
    f.text_animation(
        f"While walking down the narrow path,\n{name} found a small animal " +
    ...
    return f.choice_input(
        [
            "Try to free the animal.",
            "Avoid the animal and keep walking."
        ],
        [3, 4]
    )
