...
def not_helped_lady(name):
    ...
    f.text_animation("Press 1 or 2 to decide ...")
    return f.choice_input(
        [
            "You decide to: Try to find his way by listening",
            "You decide to: Wait in place until the fog dissipates."
        ],
        [3, 4])
...
def helped_animal(name):
    f.text_animation(
        ...
        " he led him to a secret passage that led directly" +
        " to the heart of the Kingdom of Wonders.")
    ...
    return f.choice_input(
        [
            "You decide to: Follow the magical creature.",
            "You decide to: Continued the journey alone."
        ],
        [5, 6])
...
def not_helped_animal(name):
    f.text_animation(
        ...
        " a group of enchanted trees that start to close in on him.")
