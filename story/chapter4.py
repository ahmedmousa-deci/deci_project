...
def luckily_treasure(name, lucky_thing, treasure_password):
    ...
    f.text_animation(
        f"{name} has found the treasure chest with a lock has a secret number contains from 4 digits that must be entered or another way to open it must be found.")
    ...
    while True:
        ...
        if choice == "1":
            if treasure_password_enter(treasure_password):
                f.text_animation(
                    f"{name} entered The password correctly and opened the chest")
                time.sleep(0.5)
        ...
        # thing is password paper
            ...
            f.text_animation(
                f"{name} tries to open the chest with {lucky_thing}, After many attempts, it is not impossible to open the chest")
            time.sleep(0.5)
            f.text_animation(lucky_treasure)
            break
        ...
        # if choice is break the lock with hand
            ...
            if random.choices([True, False], weights=[0.25, 0.75])[0]:
                f.text_animation(
                    f"After of {name} trying to break the lock with your hand only, He finally opened the chest")
                time.sleep(0.5)
                f.text_animation(lucky_treasure)
            else:
                f.text_animation(
                    f"After of {name} trying to break the lock with your hand only, He failed to open the chest")
            break
        ...
        # if choice is find another way
            ...
            if tool is None:
                f.text_animation(
                    f" After of {name} trying to find another way to open it, He didn't find any tool to open the chest")
                continue
            f.text_animation(
                f" After of {name} trying to find another way to open it, He finally found {tool} and he used it to open the ")
            time.sleep(0.5)
            f.text_animation(lucky_treasure)
            break
...
def using_magic_wand(name, lucky_thing, treasure_password):
    f.text_animation(
        f"{name} uses the magic wand to open the secret door, he finds the Kingdom of Wonders and its hidden treasure.")
...
def keeping_magic_wand(name):
    f.text_animation(
        f"{name} keeps the wand for emergencies, he faces a group of monsters on his way, and the wand turns out to be useless.")
    f.continue_key()
    print_lost()
...
def trying_to_find_way(name):
    f.text_animation(
        f"{name} tries to find his way by listening to the sounds of the forest, he ends up in a cave full of traps.")
    f.continue_key()
    print_lost()
...
def wait_the_fog(name, lucky_thing, treasure_password):
    f.text_animation(
        f"{name} waits in place until the fog clears, he finds his way to the Kingdom of Wonders once the fog lifts.")
    luckily_treasure(name, lucky_thing, treasure_password)
    f.continue_key()
    print_victory()
...
def follow_animal(name, lucky_thing, treasure_password):
    f.text_animation(
        f"{name} follows the magical creature, he reaches the Kingdom of Wonders and finds the treasure.")
    luckily_treasure(name, lucky_thing, treasure_password)
    f.continue_key()
    print_victory()
...
def continue_alone(name):
    f.text_animation(
        f"If {name} continues on his journey alone, he finds himself at a dead end and gets lost in the forest.")
    f.continue_key()
    print_lost()
...
def talking_trees(name, lucky_thing, treasure_password):
    f.text_animation(
        f"{name} tries to talk to the trees, a wise tree appears and helps him find the right path to the Kingdom of Wonders.")
    luckily_treasure(name, lucky_thing, treasure_password)
    f.continue_key()
    print_victory()
